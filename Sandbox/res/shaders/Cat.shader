...
void main() 
{
	...
	vec3 diffuseColor=1.0f*texture(material.texture_diffuse1,fs_in.v_texcoord).xyz*texture(toon,vec2(diffuse,diffuse)).xyz;
	

	//高光反射
	vec3 ha= normalize(CameraDir+LightDir);
	float specular = pow(max(0.4f,dot(Normal,ha)),20.0f);
	...
	//float fresnel = 0.4f*pow((1-dot(CameraDir,Normal)),2.0f);
	


	//color =vec4(gl_FragCoord.z,gl_FragCoord.z,gl_FragCoord.z,1.0f);
	//color =vec4(1.0f,1.0f,1.0f,1.0f);
	color =vec4((ambientColor+diffuseColor),1.0f);//specularColor//*(1.0f-shadowColor) //texColor;//u_color;//vec4(0.2,0.7,0.3,1.0); 
}