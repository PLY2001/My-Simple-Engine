...
Hazel::Arrow::Arrow(glm::vec3 Pos, glm::vec3 Rotation, glm::vec3 Scale, std::shared_ptr<Model>& model):Pos(Pos), Rotation(Rotation), Scale(Scale), model(model)
{
...
void Hazel::Arrow::ChangePos(glm::vec3 ChangedPos)
{
	Pos = ChangedPos;
	SetAABB();
}

glm::mat4 Hazel::Arrow::GetModelMatrix()
{
	ModelMatrix = glm::mat4(1);
	ModelMatrix = glm::translate(ModelMatrix, Pos);
	return ModelMatrix;
...
glm::vec3 Hazel::Arrow::GetAABBMaxPos(int axis)
{
	return AABBMaxPos[axis];
}

glm::vec3 Hazel::Arrow::GetAABBMinPos(int axis)
{
	return AABBMinPos[axis];
}

void Hazel::Arrow::SetAABB()
{
	for(int n = 0; n < 3; n++)
	{
